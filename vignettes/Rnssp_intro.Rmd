...
The next step is to create an NSSP user profile by creating an object of the class `Credentials`. The `Credentials` class has been designed with the R6 object system which integrates classical Object Oriented Programming concepts into R. Here, we use the `create_profile()` function to create a user profile.
